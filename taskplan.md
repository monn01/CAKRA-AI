...
- **Update (rombak UI, pasca Fase 8)**: keputusan "sengaja tidak ada signup" di atas DIBATALKAN atas permintaan user — sekarang ada halaman `src/app/(auth)/register/page.tsx` (form nama/email/kata sandi/konfirmasi, gaya sama seperti `login/page.tsx`) + `src/app/api/auth/register/route.ts` (bikin `Teacher` baru, `bcrypt.hash`, cek email duplikat via P2002). Setelah daftar, langsung auto sign-in lewat NextAuth lalu redirect ke `/dashboard`. `scripts/create-teacher.mjs` (CLI) tetap ada sebagai alternatif provisioning oleh admin sekolah (bikin banyak akun sekaligus / tanpa lewat browser), bukan lagi satu-satunya jalan.
